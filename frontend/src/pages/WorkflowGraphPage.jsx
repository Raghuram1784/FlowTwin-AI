...
export function WorkflowGraphPage() {
  ...
  const fetchGraphAndPaths = async () => {
    setLoading(true);
    setError(null);
    try {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-2 border-b border-border/60">
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant="outline" className="text-xs font-semibold">{currentWorkflow?.name || 'Workflow'}</Badge>
          </div>
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
            Left-to-Right directed graph with automatic alignment, primary success highway, rework loops, and abandonment branches
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3.5 rounded-xl border border-border bg-card shadow-xs text-xs">
      ...
      <Card className="p-2 border-border shadow-xs overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 border-b border-border mb-2">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            ...
            <span>Hover edges to view volume & rate. Click any node to open the detailed inspection Sheet.</span>
          </div>
        ...
        {loading ? (
          <div className="h-[640px] w-full flex items-center justify-center">
            <div className="flex flex-col items-center gap-3">
              ...
              <span className="text-xs text-muted-foreground">Synthesizing NetworkX directed graph & Left-to-Right layout...</span>
            </div>
          </div>
        ) : error ? (
          <div className="h-[640px] w-full flex items-center justify-center text-xs text-destructive">
            {error}
          </div>
        ) : (
          <WorkflowFlowGraph
